...
public interface SousServiceRepository extends JpaRepository<SousServiceEntity, Long> {
	...
	@Query("SELECT ss FROM SousServiceEntity ss WHERE " + "(:ville IS NULL OR ss.ville = :ville) AND "
			+ "ss.service.id IN :serviceIds AND " + "ss.id NOT IN ("
			+ "SELECT r.sousService.id FROM ReservationSousService r "
			+ "WHERE r.evenement.dateDebut != :dateDebut AND " + "(r.statut = 'EN_ATTENTE' OR r.statut = 'APPROUVEE'))")
	List<SousServiceEntity> findSousServicesDisponibles(@Param("ville") String ville,
			@Param("serviceIds") List<Long> serviceIds, @Param("dateDebut") LocalDate dateDebut);

	@Query("SELECT ss FROM SousServiceEntity ss WHERE " + "ss.service.id IN :serviceIds AND " + "ss.id NOT IN ("
			+ "SELECT r.sousService.id FROM ReservationSousService r "
			+ "WHERE r.evenement.dateDebut != :dateDebut AND " + "(r.statut = 'EN_ATTENTE' OR r.statut = 'APPROUVEE'))")
	List<SousServiceEntity> findSousServicesDisponiblesnoville(@Param("serviceIds") List<Long> serviceIds,
			@Param("dateDebut") LocalDate dateDebut);

	@Query("SELECT ss FROM SousServiceEntity ss " + "WHERE (:ville IS NULL OR ss.ville = :ville) AND " + "NOT EXISTS ("
			+ "   SELECT r FROM ReservationSousService r " + "   WHERE r.sousService = ss AND "
	...
	@Query("SELECT ss FROM SousServiceEntity ss WHERE " + "ss.service.id IN :serviceIds AND "
			+ "(ss.service.id != 5 OR ss.ville = :ville) AND " + // ville condition only applies when service.id == 5
			"ss.id NOT IN (" + "   SELECT r.sousService.id FROM ReservationSousService r "
	...
	@Query("SELECT ss FROM SousServiceEntity ss WHERE " + "ss.service.id = 5 AND " + "ss.ville = :ville AND "
			+ "ss.id NOT IN (" + "   SELECT r.sousService.id FROM ReservationSousService r "
			+ "   WHERE r.evenement.dateDebut != :dateDebut AND "
			+ "         (r.statut = 'EN_ATTENTE' OR r.statut = 'APPROUVEE')" + ")")
